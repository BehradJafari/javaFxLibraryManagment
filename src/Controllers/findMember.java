...
public class findMember {
    ...
    private void findingName(String text) {
        ...
        if (members.size()==0){
            new Alert(Alert.AlertType.ERROR,"چنین کاربری موجود نیست ",ButtonType.OK).show();
            return;
        }
    ...
    public void goToShowMember(MouseEvent mouseEvent) {
        ...
        showMemberController.member=member;
        showMember();
    }
    ...
    private void showMember() {
        ...
        Parent root = null;
        try {
            root = FXMLLoader.load(getClass().getResource("../fxmls/showMember.fxml"));
        } catch (IOException e) {
            e.printStackTrace();
        }
